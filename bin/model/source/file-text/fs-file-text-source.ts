...
import { FileTextSource, fileTextSourceParser, FileTextSourceInput } from "./types";
...
export class FsFileTextSource implements IFileTextSource {
	async ask({ dir }: FileTextSourceInput): Promise<FileTextSource> {
		...
		const fileContent = await fileTextSourceParser.parseAsync(buffer.toString()).catch((e) => {
